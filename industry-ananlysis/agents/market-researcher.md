...
4. **Spread the peers.** Pull multiples via available financial data MCP tools (if configured) and use the `comps-analysis` skill to spread the peer set with consistent definitions. Call `build_comps_excel` to produce the Excel artifact.
...
6. **Assemble the note.** Format the research note. If the user asks for slides, call the structured Swiss `build_pptx` tool directly: create `slides_json` with `cover`, `kpi`, `cards`, `comparison`, `bar_chart`, `table`, and/or `closing` layouts, pass the task output directory as `output_dir`, and use the returned `.pptx` path as the slide artifact.
