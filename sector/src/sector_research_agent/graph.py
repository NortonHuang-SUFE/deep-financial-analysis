"""Sector Research Agent - LangGraph Deep Agents graph."""
...
async def _create_agent():
    ...
    cfg = load_config()
    prompt_path = PROJECT_ROOT / "agents" / "sector.md"
    if not prompt_path.exists():
        raise FileNotFoundError(f"Agent prompt not found at {prompt_path}.")

    model = _build_model(cfg)
    mcp_tools = [] if os.getenv("SECTOR_RESEARCH_DISABLE_MCP") == "1" else await _get_mcp_tools(cfg)
    local_tools = [
        create_task_output_dir,
        write_markdown_report,
        write_json_artifact,
...
try:
    ...
    raise RuntimeError(
        f"Failed to initialise sector_research agent: {exc}\n"
        "Check sector/config.yaml, workspace .env, and installed packages."
    ) from exc
