"""Configuration loader for Morning Note.

Merge order:
  config.yaml < workspace .env / process environment

The project intentionally reads the parent workspace `.env`, not a project-local
`.env`, so secrets can be shared with adjacent agent projects.
"""
...
class Config(BaseModel):
    ...
    def load(cls, config_yaml_path: str = "config.yaml") -> "Config":
        ...
        apply_mcp_env_overrides(
            cfg.mcp,
            disable_env_var="MORNING_NOTE_DISABLE_MCP",
        )
